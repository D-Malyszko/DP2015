...
import nativeswing.common.Filter;
import nativeswing.common.UIUtils;
import nativeswing.common.UIUtils.TransparencyType;
import nativeswing.common.Utils;
import nativeswing.NativeComponentWrapper.NativeComponentHolder;
...
class NativeComponentProxyPanel extends NativeComponentProxy {
  ...
  private Rectangle[] computePeerShapeArea() {
    ...
    Rectangle[] shape = nativeswing.common.UIUtils.getComponentVisibleArea(this, new Filter<Component>() {
      public Acceptance accept(Component c) {
        if(c instanceof EmbeddedPanel) {
          return Acceptance.NO;
        }
        TransparencyType transparency = nativeswing.common.UIUtils.getComponentTransparency(c);
        switch(transparency) {
  ...
  private static class EmbeddedPanel extends Panel implements NativeComponentHolder {
    ...
    public void applyShape(Rectangle[] rectangles) {
      ...
      if(parent != null) {
        ...
        doLayout();
        nativeswing.common.UIUtils.revalidate(parent);
      }
    }
    ...
    if(isEmbeddedPanelCreated) {
      if(isProxiedFiliation) {
        ...
        if(layeredPane != oldLayeredPane) {
          ...
          nativeswing.common.UIUtils.revalidate(oldParent);
          oldParent.repaint();
          layeredPane.setLayer(embeddedPanel, Integer.MIN_VALUE);
          layeredPane.add(embeddedPanel);
          nativeComponentWrapper.restoreFromHiddenParent();
          embeddedPanel.isCrossWindowReparenting = false;
          nativeswing.common.UIUtils.revalidate(layeredPane);
          layeredPane.repaint();
          revalidate();
          repaint();
        }
